...
def _inject_settle_filters(config):
    if CONF_HAS_TARGET not in config:
        return config
    if CONF_FILTERS in config[CONF_HAS_TARGET]:
        return config
    settle = config[CONF_PRESENCE_SETTLE]
    if settle.total_milliseconds == 0:
        return config
    ent = dict(config[CONF_HAS_TARGET])
    ent[CONF_FILTERS] = validate_filters([{"settle": settle}])
    config[CONF_HAS_TARGET] = ent
    return config
